...
| `pipeline/step4_translate.py` | 基本信息表：多部分作品按**分部渲染独立表格**（一张总览表 BWV/作曲/歌词作者 + Part I~VI 各一张表，每表含该部的礼仪场合/书信/福音经文），避免单表聚合过杂 |
...
- BWV 248 基本信息表按分部拆分：总览表 + `Part I`~`Part VI` 各一张表（礼仪场合 + 书信经文 + 福音经文）
